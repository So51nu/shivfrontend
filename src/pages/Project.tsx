...
// //                         href={`https://wa.me/919987281075?text=Hi%20I%20want%20details%20for%20${encodeURIComponent(p.title)}`}
// //                         target="_blank"
// //                         rel="noreferrer"
// //                       >
...
// //                     <a href="tel:+919987281075" className="cta-call"><Phone size={18} /> Contact Sales</a>
...
//                   <div className="modal-cta">
//                     <a href="tel:+919987281075" className="cta-call"><Phone size={20} /> Contact Sales</a>
...
import { Link, useLocation } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
import { Search, Download, X, Phone, MessageSquare, ArrowRight, Menu, Building2, FileText, Sparkles, Home as HomeIcon, User, ChevronLeft, ChevronRight } from "lucide-react";
import API_BASE from "./Api";
...
type Project = {
  ...
  cover_image_url: string;
  brochure_url: string;
  gallery_urls?: Array<string | { image_url?: string; url?: string; image?: string }>;
  gallery?: Array<{ id?: number; sort_order?: number; image_url?: string; url?: string; image?: string }>;
  features: string[];
};
...
function getProjectImages(p: Project | null): string[] {
  if (!p) return [];

  const rawImages: string[] = [];

  if (p.cover_image_url) rawImages.push(p.cover_image_url);

  const galleryFromApi = Array.isArray(p.gallery) ? p.gallery : [];
  galleryFromApi
    .slice()
    .sort((a, b) => Number(a?.sort_order || 0) - Number(b?.sort_order || 0))
    .forEach((g) => {
      const url = g?.image_url || g?.url || g?.image || "";
      if (url) rawImages.push(url);
    });

  const galleryUrls = Array.isArray(p.gallery_urls) ? p.gallery_urls : [];
  galleryUrls.forEach((g: any) => {
    const url = typeof g === "string" ? g : g?.image_url || g?.url || g?.image || "";
    if (url) rawImages.push(url);
  });
  ...
  return unique.length ? unique : [HERO_IMAGES[0]];
}
...
export default function ProjectsPage() {
  ...
  // ✅ FIX: lock page scroll when project detail popup is open
  useEffect(() => {
    document.body.style.overflow = selected || galleryLightboxIndex !== null ? "hidden" : "";
    return () => {
      document.body.style.overflow = "";
    };
  }, [selected, galleryLightboxIndex]);
  ...
  const selectedImages = useMemo(() => getProjectImages(selected), [selected]);
  const selectedMainImage = selectedImages[Math.min(activeModalImage, selectedImages.length - 1)] || HERO_IMAGES[0];

  const openGalleryLightbox = (idx: number) => {
    setActiveModalImage(idx);
  ...
  const showPrevGalleryImage = () => {
    ...
    setGalleryLightboxIndex((prev) => {
      const current = prev ?? activeModalImage;
      const next = (current - 1 + selectedImages.length) % selectedImages.length;
      setActiveModalImage(next);
      return next;
    });
  };
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [galleryLightboxIndex, selectedImages.length, activeModalImage]);
    ...
    <div style={{ width: "100vw", minHeight: "100vh", background: THEME.soft, overflowX: "hidden" }}>
      ...
      <AnimatePresence>
        {selected && (
          <motion.div className="modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div className="modal-content" initial={{ scale: 0.92, y: 40 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.98, y: 20 }}>
              <button className="close-btn" onClick={() => { setGalleryLightboxIndex(null); setSelected(null); }}><X size={28} /></button>
              ...
              <div className="modal-grid">
                <div className="modal-img-area">
                  <button
                    ...
                    onClick={() => openGalleryLightbox(activeModalImage)}
                    aria-label="Open selected image in full view"
                  >
                    <img src={selectedMainImage} alt={selected.title} />
                    <div className="main-image-badge">{activeModalImage + 1} / {selectedImages.length}</div>
                    <div className="main-image-open">Click to View</div>
                  </button>
                </div>
                ...
                <div className="modal-text-area">
                  ...
                  {!!selected.features?.length && (
                    <>
                      <h4 className="amenities-title">Features & Amenities</h4>
                      <div className="features-grid">
                        {selected.features.map((f) => (
                          <div className="feat-chip" key={f}>✓ {f}</div>
                        ))}
              ...
              <div className="modal-gallery-section">
                <div className="gallery-head">
                  ...
                  <small>{selectedImages.length} Images</small>
                </div>
                ...
                <div className="modal-gallery-grid">
                  {selectedImages.map((img, idx) => (
                    <button
                      key={`${img}-${idx}`}
                      type="button"
                      className={idx === activeModalImage ? "gallery-card-thumb active" : "gallery-card-thumb"}
                      onClick={() => openGalleryLightbox(idx)}
                      aria-label={`Open project image ${idx + 1}`}
                    >
                      <img src={img} alt={`${selected.title} gallery ${idx + 1}`} />
              ...
              {galleryLightboxIndex !== null && (
                <div className="image-lightbox" onClick={closeGalleryLightbox}>
                  <button className="image-lightbox-close" onClick={closeGalleryLightbox} aria-label="Close image preview">
                    <X size={24} />
                  </button>
                  <button
                    className="image-lightbox-nav prev"
                    onClick={(e) => { e.stopPropagation(); showPrevGalleryImage(); }}
                  ...
                  <div className="image-lightbox-card" onClick={(e) => e.stopPropagation()}>
                    <img
                      src={selectedImages[galleryLightboxIndex] || selectedMainImage}
                      alt={`${selected.title} full gallery ${galleryLightboxIndex + 1}`}
                    />
                    <div className="image-lightbox-count">
                      {galleryLightboxIndex + 1} / {selectedImages.length}
                    </div>
                  </div>
                  <button
                    className="image-lightbox-nav next"
                    onClick={(e) => { e.stopPropagation(); showNextGalleryImage(); }}
      ...
      <footer className="footer">
        <div className="container footer-grid">
          <div>
            ...
            <div className="f-muted"></div>
            <div className="f-muted" style={{ marginTop: 10 }}>
              Registered Office: 202 2nd Floor Near Teli Gali Andheri East Mumbai 400072
            </div>
          </div>
  ...
  return (
    <style dangerouslySetInnerHTML={{ __html: `
      ...
      * { box-sizing:border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif; }
      ...
      .modal-content::-webkit-scrollbar-thumb{ background: rgba(0,0,0,0.22); border-radius: 999px; }
      .close-btn{ 
        position: absolute; 
        top: 16px; 
        right: 16px; 
      ...
      .gallery-card-thumb.active{
        border-color: ${brand};
        box-shadow: 0 0 0 3px rgba(179,89,0,0.16), 0 16px 34px rgba(0,0,0,0.12);
      }
      ...
      .gallery-card-thumb:hover img{ transform: scale(1.06); }
      .gallery-card-thumb span{
        position:absolute;
        left:10px;
        bottom:10px;
      ...
      /* IMAGE GALLERY POPUP - separate modal over project details */
      .image-lightbox{
        position: fixed;
        inset: 0;
        z-index: 7000;
      ...
      .image-lightbox-nav.prev{ left: 24px; }
      ...
      @media (max-width: 1100px){
        ...
        .price-slider{ grid-column: span 3; }
        .filter-actions{ grid-column: span 3; }
        .modal-overlay{ padding: 92px 16px 24px; align-items: flex-start; }
        .modal-content{ max-height: calc(100dvh - 116px); }
        .modal-grid{ grid-template-columns: 1fr; overflow: visible; }
        .modal-img-area{ overflow: visible; }
        .modal-main-image{ height: 320px; min-height: 320px; }
        .modal-gallery-grid{ grid-template-columns: repeat(3, minmax(0, 1fr)); }
        .gallery-card-thumb{ height: 110px; }
        .modal-text-area{ max-height: none; overflow: visible; }
        .features-grid{ grid-template-columns: 1fr; }
      }
      ...
      @media (max-width: 768px){
        ...
        .filter-tabs button{ width: 100%; }
        .projects-grid{ grid-template-columns: 1fr; }
        .modal-overlay{ padding: 90px 10px 18px; }
        .modal-content{ width: 98vw; max-height: calc(100dvh - 108px); border-radius: 16px; }
        .modal-main-image{ height: 240px; min-height: 240px; }
        .modal-gallery-section{ padding: 16px; }
        .modal-gallery-grid{ grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }
        .gallery-card-thumb{ height: 105px; }
        .modal-text-area{ padding: 24px; }
        .modal-text-area h2{ font-size: 28px; }
        .price-label{ font-size: 20px; }
        .main-image-open{ display:none; }
        .image-lightbox{ padding: 18px; }
        .image-lightbox-card{ width: 100%; height: 72vh; }
        .image-lightbox-nav{ width: 46px; height: 46px; }
        .image-lightbox-nav.prev{ left: 10px; }
        .image-lightbox-nav.next{ right: 10px; }
        .image-lightbox-close{ top: 12px; right: 12px; width: 44px; height: 44px; }
      }
